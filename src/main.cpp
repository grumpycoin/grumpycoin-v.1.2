...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uGRUMP
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 GRUMP if made of 50 GRUMP inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Go back the full period unless it's the first retarget after genesis. Code courtesy of Art Forz
        ...
        // Go back by what we want to be nReTargetHistoryFact*nInterval worth of blocks
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    /* current difficulty formula, grumpycoin - kimoto gravity well */
...
unsigned int static KimotoGravityWell_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
	...
	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		PastDifficultyAveragePrev = PastDifficultyAverage;
        //Explot fix https://bitcointalk.org/index.php?topic=552895
        if (LatestBlockTime < BlockReading->GetBlockTime()) {
                        if (BlockReading->nHeight > KGW_BLOCK_START_EXPLOITFIX)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (fTestNet) {
        ...
        /*Clarv, added KGW_BLOCK_START*/
        if (pindexLast->nHeight+1 >= KGW_BLOCK_START && pindexLast->nHeight+1 < KGW_BLOCK_START_EXPLOITFIX)
          { DiffMode = 2; }
        /*Clarv, added KGW_BLOCK_START_EXPLOITFIX*/
        else if (pindexLast->nHeight+1 >= KGW_BLOCK_START_EXPLOITFIX)
          { DiffMode = 3; }
	}
...
void ThreadScriptCheck() {
    RenameThread("grumpycoin-scriptch");
    ...
    for (unsigned int i=0; i<vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (fStrictPayToScriptHash)
            {
                ...
                // an Redibly-expensive-to-validate block.
...
void GenerateGrumpyCoins(bool fGenerate, CWallet* pwallet)
{
